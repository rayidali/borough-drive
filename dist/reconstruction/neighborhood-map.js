...
export function createNeighborhoodMap({data,onTravel,getPosition}){
 ...
 function draw(canvas,full=false){
  ...
  for(const b of data.buildings){c.fillStyle=b.core?'#c2a575':b.facadeSpec?.observed?'#ac9974':'#596051';c.beginPath();for(const ring of [b.p,...(b.holes||[])]){ring.forEach((p,i)=>{const q=point(...p);i?c.lineTo(...q):c.moveTo(...q);});c.closePath();}c.fill('evenodd');}
 ...
 function placePosition(b,place){
  const streetKey=s=>s.toLowerCase().replace(/\b1st\b/g,'first').replace(/\b2nd\b/g,'second');
  let candidates=b.frontages.filter(f=>f.street===b.facadeSpec?.frontStreet);
  if(!candidates.length)candidates=b.frontages.filter(f=>streetKey(b.address).includes(streetKey(f.street)));
  if(!candidates.length)candidates=b.frontages;
  const frontage=place?b.frontages[place.frontageIndex]:candidates.reduce((a,b)=>!a||b.length>a.length?b:a,null);
  if(!frontage)return b.center;
  const along=place?.along??frontage.length/2,x=frontage.x+frontage.rx*along,z=frontage.z+frontage.rz*along,road=data.roads.find(r=>r.name===frontage.street);
  return road?(road.axis==='avenue'?[road.a[0],z]:[x,road.a[1]]):[x,z];
 }
 for(const b of data.buildings){
  if(b.address)places.set(b.address,placePosition(b));
  for(const p of b.businesses||[])if(p.renderName)places.set(p.name+' · '+(p.mappedAddress||b.address||p.street),placePosition(b,p));
 }
 for(const name of [...places.keys()].sort((a,b)=>a.localeCompare(b))){const option=document.createElement('option');option.value=name;options.append(option);}
 document.querySelector('#place-form').addEventListener('submit',e=>{e.preventDefault();const q=search.value.trim().toLocaleLowerCase(),entry=[...places].find(([name])=>name.toLocaleLowerCase()===q)||[...places].find(([name])=>name.toLocaleLowerCase().includes(q));if(!q||!entry){search.setCustomValidity('Choose a listed place or street address.');search.reportValidity();return;}search.setCustomValidity('');onTravel(...entry[1]);dialog.close();refresh();});
