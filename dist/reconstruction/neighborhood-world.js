...
export function createNeighborhoodWorld(data){
 ...
 const sidewalkObjects=data.storefrontObstacles||[];
 function hitsSidewalkObject(x,z,margin=0){return sidewalkObjects.some(p=>{const dx=x-p.x,dz=z-p.z;return Math.abs(dx*p.rx+dz*p.rz)<p.halfWidth+margin&&Math.abs(-dx*p.rz+dz*p.rx)<p.halfDepth+margin;});}
 ...
 function walkable(x,z){if(x<bounds[0]||x>bounds[2]||z<bounds[1]||z>bounds[3]||hitsSidewalkObject(x,z,.22))return false;for(const [a,b] of [[0,0],[.24,0],[-.24,0],[0,.24],[0,-.24]])if(buildingAt(x+a,z+b))return false;return true;}
 ...
 function start(){return spawn(data.avenues.find(a=>a[0]==='First Avenue')[1],data.streets.find(s=>s[0]==='East 7th Street')[1]);}
 return {buildingAt,roadAt,nearestRoad,carCollides,walkable,spawn,start};
}
