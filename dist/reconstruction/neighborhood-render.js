...
const COLORS={'warm brick':0x825944,'salmon brick':0x9c7162,'buff brick':0xb1a080,'charcoal brick':0x575653,'aged brownstone':0x796050,'limestone facade':0xb6af9c,'painted ivory':0xc7c4b7,'painted grey':0x858880,'weathered red':0x7e4d3b,'red brick':0x875440,'dark red brick':0x754736,'pale render':0xb8b5a7,'cream stone':0xb6aa89,'ochre brick':0x927454,'orange stucco':0xa86243};
export async function createNeighborhoodRenderer({scene,loader,renderer,data,core,asset,onChange,onStatus}){
 ...
 function box(x,y,z,w,h,d,m,angle=0){const g=new THREE.BoxGeometry(w,h,d);g.rotateY(angle);g.translate(x,y,z);geometry(g,m);}
 const asphalt=mat('asphalt',0x404441),pavement=mat('sidewalk',0x9b9a8c),curb=mat('curb granite',0xa9aaa1),seam=mat('concrete seam',0x64665f),white=mat('road paint',0xd4d0b7),green=mat('bike green',0x41694f),yellow=mat('yellow paint',0xcda651);
 ...
 for(const r of data.roads){
  if(r.axis==='avenue'){
   const x=r.a[0];for(let zi=0;zi<sts.length-1;zi++){
    ...
    if(r.name!=='Avenue A'){
     const side=r.name==='First Avenue'?-1:1,bike=x+side*(r.halfWidth-1.35);
     box(bike,.008,(z0+z1)/2,2.45,.009,z1-z0,green);
     for(const dx of [-1.28,1.28])box(bike+dx,.016,(z0+z1)/2,.10,.008,z1-z0,white);
     for(let z=z0;z<z1;z+=4.2)box(x+side*6.4,.021,z,2.15,.006,.10,white,.36*side);
    }
 ...
 for(const tile of data.tiles){
  ...
  for(const b of data.buildings.filter(b=>b.tile===tile.id&&!b.core)){
   ...
   const shape=new THREE.Shape(b.p.map(p=>new THREE.Vector2(p[0],-p[1]))),g=new THREE.ExtrudeGeometry(shape,{depth:b.renderHeight||spec.height||b.height,bevelEnabled:false,steps:1});g.rotateX(-Math.PI/2);g.translate(0,.17,0);
 ...
 function prepare(root){root.traverse(o=>{if(!o.isMesh)return;o.castShadow=true;o.receiveShadow=true;const materials=Array.isArray(o.material)?o.material:[o.material];o.material=materials.map(m=>{const key=m.name;if(materialCache.has(key)){const known=materialCache.get(key);if(m!==known)m.dispose();return known;}for(const field of ['map','normalMap','roughnessMap','metalnessMap'])if(m[field]){const t=m[field],k=field+':'+t.name;if(t.name&&textureCache.has(k)){m[field]=textureCache.get(k);t.dispose();}else if(t.name)textureCache.set(k,t);}m.envMapIntensity=.75;if(m.map)m.map.anisotropy=maxAniso;if(m.normalMap)m.normalScale.set(.35,.35);materialCache.set(key,m);return m;});if(!Array.isArray(o.material)||o.material.length===1)o.material=o.material[0];const ms=Array.isArray(o.material)?o.material:[o.material];if(ms.every(m=>m.name==='store glass'))o.castShadow=false;});return root;}
 const distance=(x,z,b)=>Math.hypot(Math.max(b[0]-x,0,x-b[2]),Math.max(b[1]-z,0,z-b[3]));
 let queue=[],active=0,alive=true;
 function pump(){while(alive&&active<2&&queue.length){const s=queue.shift();if(s.root||s.promise)continue;active++;onStatus?.('Loading nearby streets…');s.promise=loader.loadAsync(asset(s.tile.url)).then(g=>{s.root=prepare(g.scene);scene.add(s.root);proxyGroups.get(s.tile.id).visible=false;onChange?.();}).catch(error=>{s.failedAt=performance.now();console.warn('Neighborhood section unavailable',s.tile.id,error);onStatus?.('Some street details could not load. They will retry.');}).finally(()=>{s.promise=null;active--;if(!active&&!queue.length)onStatus?.('');pump();});}}
