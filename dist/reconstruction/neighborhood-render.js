...
const COLORS={'painted blue':0x2b6888,'painted ochre':0xc49a34,'warm brick':0x825944,'salmon brick':0x9c7162,'buff brick':0xb1a080,'charcoal brick':0x575653,'aged brownstone':0x796050,'limestone facade':0xb6af9c,'painted ivory':0xc7c4b7,'painted grey':0x858880,'weathered red':0x7e4d3b,'red brick':0x875440,'dark red brick':0x754736,'pale render':0xb8b5a7,'cream stone':0xb6aa89,'ochre brick':0x927454,'orange stucco':0xa86243};
export async function createNeighborhoodRenderer({scene,loader,renderer,data,core,asset,onChange,onStatus}){
 ...
 function prepare(root){root.traverse(o=>{if(!o.isMesh)return;o.castShadow=true;o.receiveShadow=true;const materials=Array.isArray(o.material)?o.material:[o.material];o.material=materials.map(m=>{const key=materialKey(m);if(materialVariants.has(key)){const known=materialVariants.get(key);if(m!==known)m.dispose();return known;}for(const field of ['map','normalMap','roughnessMap','metalnessMap'])if(m[field]){const t=m[field],k=field+':'+t.name;if(t.name&&textureCache.has(k)){m[field]=textureCache.get(k);t.dispose();}else if(t.name)textureCache.set(k,t);}m.envMapIntensity=.75;if(m.map)m.map.anisotropy=maxAniso;if(m.normalMap)m.normalScale.set(.35,.35);materialVariants.set(key,m);return m;});if(!Array.isArray(o.material)||o.material.length===1)o.material=o.material[0];const ms=Array.isArray(o.material)?o.material:[o.material];if(ms.every(m=>m.name==='store glass'))o.castShadow=false;});return root;}
