...
// --user-data-dir, then: node scripts/review-storefronts.mjs [output directory]
import fs from 'node:fs/promises';
import path from 'node:path';
import assert from 'node:assert/strict';
const out=path.resolve(process.argv[2]||'renders/storefront-review');
...
const report={date:new Date().toISOString(),url:'http://127.0.0.1:5173',errors:[],failedRequests:[],screenshots:[]};
...
on('Debugger.paused',async e=>{
 try{await send('Debugger.evaluateOnCallFrame',{callFrameId:e.callFrames[0].callFrameId,expression:`globalThis.__boroughReview={
  ...
  graphics:()=>({renderer:renderer.getContext().getParameter(renderer.getContext().getExtension('WEBGL_debug_renderer_info').UNMASKED_RENDERER_WEBGL),pixelRatio:renderer.getPixelRatio(),ao:ao.enabled})
...
for(const entry of schedule.frontages){
 ...
 if(entry.design.blades?.length){
  await evaluate(`__boroughReview.look(${tx-f.rz*d+f.rx*6},${tz+f.rx*d+f.rz*6},${tx},3.1,${tz})`);
  await screenshot(entry.id+'-blade');
 }
}
for(const entry of schedule.elevations){
 const b=data.buildings.find(b=>b.id===entry.buildingId),f=b.frontages.find(f=>f.street===entry.street);
 const tx=f.x+f.rx*f.length/2,tz=f.z+f.rz*f.length/2,h=b.renderHeight||b.height;
 const d=Math.max(16,Math.min(24,h*.95));
 await evaluate(`__boroughReview.look(${tx-f.rz*d},${tz+f.rx*d},${tx},${h*.48},${tz})`);
 await until(`__boroughReview.pose().tiles.includes(${JSON.stringify(b.tile)})`);
 await screenshot('elevation-'+entry.id);
}
...
await fetch('http://127.0.0.1:9222/json/close/'+tab.id);ws.close();assert.equal(report.errors.length,0);assert.equal(report.failedRequests.length,0);
