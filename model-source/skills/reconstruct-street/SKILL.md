...
**Current September 23 scope:** First Avenue–Avenue A on East 7th remains the bounded assignment. The user explicitly requests browser Street View observation and correct number placements; the [revision 07 report](../../SEVENTH-ENGINE-07.md) and [observation ledger](../../east-seventh-browser-07.json) retain actual inspection and uncertainty. Read the current handoff for final validation/save state. Revision 04 remains the user-endorsed visual reference. Preserve the published compact interface and audio; no additional street or publication is implied. Earlier source choices do not override a later explicit browser-observation request.
