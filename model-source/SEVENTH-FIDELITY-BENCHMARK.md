...
**September 23 continuation:** the bounded First Avenue–Avenue A [revision 07](SEVENTH-ENGINE-07.md) adds browser-observed frontage and address details. Its source/render reviews do not change the calibrated acceptance criteria below or certify the block as 1:1. The [current handoff](../CODEX-HANDOFF.md) records exact local save and validation state.

**Saved revision 04 fidelity scope (September 15):** East 7th between First and Second Avenues only: **39 mapped Seventh-facing building records plus the vacant church/fire-site street edge at the Second Avenue end**. The two avenue-addressed corner faces are included. The previous 82-frontage scope below describes revision 03's full slice, which remains preserved outside this pass. That pass's stricter request includes construction/dining sheds, fixed street fixtures and all visible basement businesses; a building-only inventory is insufficient. Each source date must travel with temporary features. Latest available September 2024/April 2026 views and July 2026 existing-condition architectural material do not establish an identical September 2026 streetscape.

**September 21 closing state:** the later [interface pass](INTERFACE-PASS-05.md) is published and preserves this geometry. No new photographic acceptance, worker qualification or street task was added. The 39 source/elevation reviews and zero calibrated multi-view passes remain the saved state. Resume from the [current handoff](../CODEX-HANDOFF.md) and the user's next request; the proposed work below does not restart automatically.
...
The user endorsed Seventh revision 04's quality as the reference for future streets and initially requested reusable instructions before implementation. The [portable reconstruction skill](skills/reconstruct-street/SKILL.md) translates that visual direction into bounded work packets and detailed review gates. This supersedes earlier wording that no current visual reference can be used for cheaper-model work. It does not accept every individual feature, approve the numerical photographic thresholds below or establish calibrated multi-view acceptance. Future authorized work can use revision 04 as its visual standard while retaining the stricter 1:1 target and explicit unresolved details. No worker-model cost/quality qualification has run.
...
Passing requires the following work for **every property and the intervening street/site features**, including both avenue-addressed corner faces and the vacant church/fire-site edge:

1. **Define the dated real-world target and cover it with evidence.** Find sufficiently clear frontal, oblique and entrance views; resolve occlusions and conflicting dates. Record the date of signs, tenants, construction, sheds and other changing details. The mixed 2024/2026 references currently support a reconstruction with dated exceptions, not a single verified September 2026 snapshot. An unobserved required detail remains unresolved. Reference discovery is part of the work; the future process must not depend on the user uploading every image.
2. **Finish observed geometry and appearance.** Correct each floor's window layout, actual door shape/panels, basement or raised access, ramps/railings, facade proportions, cornices, fire escapes, shop divisions, lettering, material pattern/scale and fixed street objects. A checklist must link each required feature to its evidence and modeled result. Broad resemblance or a supported business name is insufficient.
3. **Compare photographs and game views from equivalent cameras.** Align stable building landmarks, then compare side by side and with overlays. Check an independent second viewpoint to expose wrong depth or proportions that a single camera can hide; add readable entrance/sign closeups. Keep the hard count/type/identity gate separate from the numerical alignment gate below. The 1.5% mean / 3% maximum / 1% entrance values are **proposed project image-space tolerances**, not industry standards or user-approved criteria. Photographic agreement at those tolerances does not establish literal world-space 1:1 dimensions; that stronger claim requires dimensional reference measurements and checks with documented uncertainty.
4. **Close the discrepancies and retain a reviewable record.** For each property and street/site segment, save its dated sources, annotated comparisons, remaining unknowns, reviewer decision and exact model/game version. Required unknowns or known mismatches prevent a complete pass. The proposed first comparison is the existing **50 and 48½ East 7th** pair; after a concrete benchmark is reviewed and accepted, all remaining records must pass the same gates. Acceptance of the earlier First & Seventh minimum does not establish stricter 1:1 acceptance, and passing one pair does not certify the block.
5. **Preserve the game experience separately.** Lofi lighting and weather can coexist with faithful architecture, but review must make architectural color/material identity and details legible. Run browser performance and preservation checks on the resulting package. A stronger server, GPU rental or engine migration is not a prerequisite for these fidelity checks, and photographic acceptance does not establish performance or multiplayer capacity.

This clarification and proposed sequence were documented at the user's closing request. They do not record user approval of a new facade, date policy or tolerance, and no new modeling, acquisition or pipeline job was started. The [current handoff](../CODEX-HANDOFF.md) controls the next authorized action.
