...
**Superseded as the active direction:** later on September 10 the user chose smooth browser-first play and deferred Unreal. Follow [browser pass 09](BROWSER-PASS-09.md) and [the bounded block workflow](BLOCK-WORKFLOW.md). Reaffirmed in the September 12 continuation: new implementation is paused while the automatic block process is brainstormed. The assessment below remains historical; it is not permission to migrate.
