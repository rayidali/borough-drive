...
The generated `neighborhood-business-audit.json` covers 369 mapped candidates plus reviewed additions: 212 supported names and 167 excluded/unresolved records. Matching uses exact addresses, mapped positions, municipal identities and individual corrections; each name selects a specific street frontage. Multiple tenants receive estimated separate units. Website address evidence, recent inspections, retail licenses and contrary evidence retain their own dates. A currently reachable website can still be stale: Ferns is excluded because a newer broker listing identifies a former tenant. A name in this inventory does not certify its sign design, partition dimensions or continued operation.
