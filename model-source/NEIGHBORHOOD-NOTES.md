...
**Current checkpoint, September 9:** The owner accepted [First & 7th revision 06](FIRST-SEVENTH-PASS-06.md) as the minimum completion standard for later work. [Performance pass 07](PERFORMANCE-PASS-07.md) retains all its geometry/material sources and optimizes the runtime. Source observations remain in [first-and-seventh.json](first-and-seventh.json), `first_seventh_kit.py` and [storefront-details.json](storefront-details.json). Publication is authorized; read the [handoff](../CODEX-HANDOFF.md) and [session log](../SESSION-LOG.md) for exact commits, push/deployment checks and next action. Wider East Village work and eventual Manhattan/NYC processing are deferred to a later plan.
