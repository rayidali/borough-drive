...
**Superseded compute decision:** later September 14 the user said a GPU machine is unavailable and authorized the [Godot browser slice](../engine/first-seventh/README.md), now published with [interface pass 05](INTERFACE-PASS-05.md) at [DriveAround.nyc](https://www.drivearound.nyc/). The [current handoff](../CODEX-HANDOFF.md) takes precedence. The plan below is a historical Unreal option; its pending-question, local-only and Godot-pause statements are not current instructions. No GPU hosting was purchased, and no Unreal work is assigned at session close.
