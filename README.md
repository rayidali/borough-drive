...
**September 14:** a bounded First & 7th Unreal browser slice is now requested, with lofi graphics, weather, multiple driving cameras and an eventual goal of approximately 500 concurrent independent players. The existing published game remains unchanged. [The pilot plan](model-source/UNREAL-BROWSER-PILOT.md) records the GPU streaming architecture and pending machine/budget decision. The initial Godot experiment is paused and incomplete. Read [the latest handoff](CODEX-HANDOFF.md) before acting on the historical September 12 discussion below.

Read [AGENTS.md](AGENTS.md), the current [handoff](CODEX-HANDOFF.md), the latest [session log](SESSION-LOG.md), and [START-HERE.md](START-HERE.md) before continuing an existing session. First & 7th revision 06 is the accepted minimum standard; [browser pass 09](model-source/BROWSER-PASS-09.md) records current presentation and performance, following [pass 07](model-source/PERFORMANCE-PASS-07.md). The handoff distinguishes local commits, pushing to main and a verified live deployment. **September 12 resume state: brainstorming only.** The user wants the future process to find dated reference views itself, build and review small tasks, save progress, and eventually cover every part of the map. The offline inventory/packets exist; automatic acquisition, paid workers and the coordinator do not. Continue that discussion without implementing or launching jobs.
