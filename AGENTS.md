...
- Latest direction, reaffirmed for the September 12 handoff: **brainstorm only; do not implement the pipeline or further game changes until the user instructs otherwise.** Saved source `c76dcce` is published; publication documentation `8792ce4` was pushed. The requested future process must find reference views itself and retain progress across sessions, not depend on the user manually supplying each image.
