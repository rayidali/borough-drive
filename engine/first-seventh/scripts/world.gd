...
func _ready():
	...
	sidewalk = S.material(Color("ada698"))
...
func build_streets():
	S.solid_box(self,Vector3(-95,-.22,-26),Vector3(440,.4,265),null)
	S.box(geometry,Vector3(-95,-.015,-26),Vector3(440,.025,265),asphalt)
	# Original mapped road centerlines and widths; the sidewalk fills the intervening blocks.
	var x_ranges = [[-305.0,-239.75],[-218.25,-10.75],[10.75,115.0]]
	...
	for z in [0.0,-77.9]:
		for x in range(-257,75,12):
			if abs(x)<17 or abs(x+229)<17: continue
	...
	for x in [-280,88]: S.solid_box(self,Vector3(x,10,-31),Vector3(1,30,190),null)
	for z in [-130,67]: S.solid_box(self,Vector3(-95,10,z),Vector3(380,30,1),null)
...
func build_props():
	var trunk = S.material(Color("786251"))
	var foliage = [S.material(Color("668477")),S.material(Color("899778")),S.material(Color("a5a57e"))]
	...
	for z in [0.0,-77.9]:
		for side in [-1,1]:
			for x in [-204.0,-154.0,-107.0,-63.0,55.0]:
				...
				S.cylinder(geometry,p+Vector3(0,1.65,0),.10,3.3,trunk)
				S.box(geometry,p+Vector3(0,.15,0),Vector3(1.65,.02,1.5),S.material(Color("6c7364")))
				for j in range(5):
					var m = SphereMesh.new()
					m.radius = rng.randf_range(.83,1.35)
					m.height = m.radius*1.8
					m.radial_segments = 7
					m.rings = 4
					var pos = p+Vector3(rng.randf_range(-.8,.8),rng.randf_range(3.3,4.5),rng.randf_range(-.8,.8))
					S.mesh(geometry,pos,m,foliage[j%foliage.size()])
