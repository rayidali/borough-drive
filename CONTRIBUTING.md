...
The owner accepted First & 7th revision 06 as the minimum completion standard on September 9. [Performance pass 07](model-source/PERFORMANCE-PASS-07.md) preserves the models and improves runtime speed. The later [pass 08](model-source/FIRST-SEVENTH-PASS-08.md) refines this corner using dated Street View observations. The September 21 interface is preserved by the subsequent street refinements; see the [handoff](CODEX-HANDOFF.md) and [interface report](model-source/INTERFACE-PASS-05.md) for source/publishing state. The user-endorsed [revision 07](model-source/SEVENTH-ENGINE-07.md) refines First Avenue–Avenue A from browser Street View observations of all 37 frontages, with observed address placements and explicit remaining evidence gaps. Production publication is complete through checkpoint `78aa716`, with both CI jobs and Vercel passing. The user endorsed browser Street View as the preferred reference method; the session is closed and no other street is assigned. For street work, use the [portable reconstruction skill](model-source/skills/reconstruct-street/SKILL.md); for separate branding/music proposals, read the [experience brief](docs/DRIVEAROUND-EXPERIENCE-BRIEF.md). The September 12 automatic block process remains historical planning, and the offline planner does not run it.
...
For historical browser-first planning, read [pass 09](model-source/BROWSER-PASS-09.md) and [the block workflow](model-source/BLOCK-WORKFLOW.md). `npm run workflow:verify` tests the offline planner; Python 3 is needed for these optional workflow tools. Worker packets propose edits for one facade and cannot self-accept or publish. The Google API key is not a bulk/derivative-use permission grant. The current published interface and exact resume state remain in the [handoff](CODEX-HANDOFF.md).
