...
The owner accepted First & 7th revision 06 as the minimum completion standard on September 9. [Performance pass 07](model-source/PERFORMANCE-PASS-07.md) preserves the models and improves runtime speed. The later [pass 08](model-source/FIRST-SEVENTH-PASS-08.md) refines this corner using dated Street View observations. The September 21 interface package is now published on the existing revision 04 architecture; see the [handoff](CODEX-HANDOFF.md) and [interface report](model-source/INTERFACE-PASS-05.md) for source/publishing state. No next street is assigned. For future street work, use the [portable reconstruction skill](model-source/skills/reconstruct-street/SKILL.md); for separate branding/music proposals, read the [experience brief](docs/DRIVEAROUND-EXPERIENCE-BRIEF.md). The September 12 automatic block process remains historical planning, and the offline planner does not run it.
