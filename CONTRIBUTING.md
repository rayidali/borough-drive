...
The owner accepted First & 7th revision 06 as the minimum completion standard on September 9. [Performance pass 07](model-source/PERFORMANCE-PASS-07.md) preserves the models and improves runtime speed. The later [pass 08](model-source/FIRST-SEVENTH-PASS-08.md) refines this corner using dated Street View observations. Read its report and the handoff for source/publishing state. As of the September 12 handoff, an economical process that finds references and builds/reviews blocks automatically is being brainstormed. The current offline planner does not run that process. The user has paused new implementation; continue the discussion before starting a pilot, expansion or unattended jobs.
