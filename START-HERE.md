...
**Current close — September 21 interface update:** the included Seventh build replaces the large left introduction with a compact bottom launch panel, bundled Inter typography, the supplied logo and a quieter charcoal/sage loading screen, HUD and pause menu. Interface source/runtime `b194dc6` is published; publication checkpoint `533570e` passed both CI jobs and Vercel deployment, with route evidence in the [handoff](CODEX-HANDOFF.md). Open **http://127.0.0.1:5173/** after `npm run dev`. Escape → Controls reveals the full keyboard/mouse guide. See the [interface report and captures](model-source/INTERFACE-PASS-05.md). Models and audio are unchanged.

The earlier September 21 [portable reconstruction skill](model-source/skills/reconstruct-street/SKILL.md) is published and ready for any assistant. It records the user-endorsed Seventh visual standard, detailed quality checks and small worker/reviewer packets. No next street is assigned. The [experience brief](docs/DRIVEAROUND-EXPERIENCE-BRIEF.md) retains the remaining icon, branding and louder, calming music work.

**Architecture remains revision 04:** Seventh uses the Vercel root route (`/`) and remains directly available at `/seventh/`. It has a live address label on the First–Second block, photo-guided corrections and camera movement independent of the car. Right-drag to orbit/look, Shift + right-drag or middle-drag to pan up toward roofs, wheel to zoom, and V to recenter the camera. C selects the driving view. The legacy ten-block game remains at `/index.html`. Drift, sound, weather and the full Seventh-through-Avenue-A route remain. See [controls/rebuild](engine/first-seventh/README.md), [revision 04 evidence and limits](model-source/SEVENTH-ENGINE-04.md) and [fidelity benchmark](model-source/SEVENTH-FIDELITY-BENCHMARK.md). No next street is assigned; the block is not certified 1:1.
