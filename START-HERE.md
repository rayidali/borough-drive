...
**September 21 interface update:** the included Seventh build replaces the large left introduction with a compact bottom launch panel, bundled Inter typography, the supplied logo and a quieter charcoal/sage loading screen, HUD and pause menu. Open **http://127.0.0.1:5173/** after `npm run dev`. Escape → Controls reveals the full keyboard/mouse guide. See the [interface report and captures](model-source/INTERFACE-PASS-05.md) and [handoff](CODEX-HANDOFF.md) for exact local/published state. Models and audio are unchanged.
