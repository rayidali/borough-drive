...
**September 23 close:** begin the next session with [CODEX-HANDOFF.md](CODEX-HANDOFF.md), the latest [session log](SESSION-LOG.md), and Git status/history. Revision 07 is published, including the preferred browser Street View method. The next task comes from the user; no rebuild, reference acquisition or surviving browser/server process is required to resume.

**Current revision 07 — East 7th, First Avenue to Avenue A:** browser Street View observations cover all 37 frontage records, with explicit door/number/shop/window corrections and the 108 gated street edge. [Current report and review state](model-source/SEVENTH-ENGINE-07.md); [exact save/test/push state](CODEX-HANDOFF.md). The user endorsed this result, and it is published to main/production with CI and live byte verification recorded in the handoff. Publication does not establish photographic 1:1 acceptance.

Run `npm run dev` and open **http://127.0.0.1:5173/**. `/seventh/` is the same included export; `/index.html` remains the original ten-block game. Drive east from First along Seventh to review this pass. No rebuild or API key is needed to play. Right-drag to orbit/look, Shift + right-drag or middle-drag to pan, wheel to zoom, V to recenter, and C to change POV. Escape → Controls opens the full guide. [Controls/rebuild](engine/first-seventh/README.md).

The published September 21 [compact interface](model-source/INTERFACE-PASS-05.md), west-block revision 04, accepted corner and audio are preserved. The [portable reconstruction skill](model-source/skills/reconstruct-street/SKILL.md) remains the source/review workflow. The [experience brief](docs/DRIVEAROUND-EXPERIENCE-BRIEF.md) retains separate branding/music proposals. Revision 06 used public archives after billing-disabled API metadata responses. The later user request explicitly authorizes ordinary browser Street View observation for revision 07; no paid imagery request was made. The skill now prefers this browser-reference method for future authorized streets. No paid acquisition retry or additional street is assigned; the handoff records the completed publication and closing state.
...
**September 12 historical instruction: brainstorming only.** September 14 supersedes that pause for the bounded game slice above. The desired future batch process must find useful reference views itself, process each block’s facades and street details, review the results and retain progress without depending on this chat or manual image uploads. Automatic acquisition, a persistent execution ledger and the worker/build/review coordinator are not implemented.

**Historical September 12 planning note:** the direction was browser-first with Unreal deferred. [Browser pass 09](model-source/BROWSER-PASS-09.md) improved the corner's presentation and rendering; [the block workflow](model-source/BLOCK-WORKFLOW.md) prepares small offline worker packets and a finite coverage inventory. No paid AI batch or wider building pass has started. Google API access does not establish permission for bulk/derived-content processing; any future workflow requires suitable imagery rights.
...
For a new session, open this repository and start with the current checkpoint in [CODEX-HANDOFF.md](CODEX-HANDOFF.md) and the latest entry in [SESSION-LOG.md](SESSION-LOG.md). The log records completed work, the exact gameplay commit, publishing status, unfinished work and next steps; this close leaves no assigned next street. [AGENTS.md](AGENTS.md) instructs coding agents to read and maintain these records as they work.
